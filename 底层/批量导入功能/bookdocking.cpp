...
bool bookdocking::multiadd(int num, string ISBN, string name, string author, string type, string borrowtime, string returntime, string histroy, string onsheelf, string isovertime)
{
	secData.close();
	int tid = 0;
	int tim = num;
	...
	while (tim) {
		if (!bookexist(tid)) {
			tim -= 1;
			bookadd(tid, ISBN, name, author, type, borrowtime, returntime, histroy, onsheelf, isovertime);
		}
		tid += 1;
	}
