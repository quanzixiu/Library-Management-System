...
public:
	...
	bool multiadd(int num, string ISBN, string name, string author, string type, string borrowtime, string returntime, string histroy, string onsheelf, string isovertime);

	bool search_delete(int id);
	bool file_update();
	vector<string> search(string method, string value);
	bool search(string method, string value, vector<string>* rec);
	bool search(vector<string>* rec);
