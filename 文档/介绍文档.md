...
![image-20201013090649894](D:/Github/Library-Management-System/文档/img/tech1_-1.png)
...
构造函数，确定cache大小，将所有有效位置为0。

（2）bool select(const int key, string &value)

从cache中取值，将取回的值放在value中，返回值表示是否取值成功。

（3）int insert(const int key, const string &value,int *oldKey,string *oldValue)

向cache中插入值，若cache中原来有值且被修改过，则将原来的key和value存入*oldKey和*oldValue。返回值含义为0表示插入失败，1表示原来没有值或值没有被修改，2表示原来有值且被修改过。

（4）void remove(const int key)

从cache中删除，若cache中存有key且有效位为true，则将有效位置为false。

（5）bool update(const int key, string &value)

更新cache中的值，若cache中没有key，返回false，否则更新值且将dirty置为true。

（6）void save(vector<int> *key, vector<string> *value)

将cache中的内容写会，所有没有保存修改的key和value存入*key和*value，交由索引文件和数据文件保存。
