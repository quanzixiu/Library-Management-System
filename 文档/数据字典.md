...
底层函数返回值：返回用户对象  //逻辑层可以自行提取用户密码与输入密码进行对比
...
调用底层函数：bookadd(string ISBN,string name,string author,string publisher,string type,string id,string borrowtime,string returntime,string history, bool onsheelf,bool isovertime;)
