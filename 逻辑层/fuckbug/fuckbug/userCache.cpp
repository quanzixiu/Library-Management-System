...
bool userCache::select(const int key, string &value)
{
	int set = GETSET(key);
	int tag = GETTAG(key);
...
int userCache::insert(const int key, const string &value, int *oldKey, string *oldValue)
{
	int set = GETSET(key);
	int tag = GETTAG(key);
	if (tag == GETTAG(cache[set].key) && cache[set].valid)
...
void userCache::remove(const int key)
{
	int set = GETSET(key);
	int tag = GETTAG(key);
...
bool userCache::update(const int key, string &value)
{
	int set = GETSET(key);
	int tag = GETTAG(key);
	if (tag != GETTAG(cache[set].key) || !cache[set].valid)
...
void userCache::save(vector<int> *key, vector<string> *value)
{
	for (int i = 0; i < CACHESIZE; ++i)
	{
