...
{
	scale = 5;
	indexFile.open("userindex.txt", ios::in | ios::out | ios::binary);
	dataFile.open("userdata.txt", ios::in | ios::out | ios::binary);
	logFile.open("userlog.txt", ios::trunc);
	if (!indexFile)
	{
		indexFile.close();
		dataFile.close();
		ofstream index("userindex.txt", ios::binary);
		ofstream data("userdata.txt", ios::binary);
		int firstEmpty = 8;
...
void userDatabase::dataFile_find(const int dataAddress, string &value)
{
	char tmp[56];
	dataFile.clear();
	dataFile.seekg(dataAddress, ios::beg);
	dataFile.read((char *)tmp, 56);
	value = tmp;
	logFile << "[dataFile_find] dataAddress:" << dataAddress << " value:" << value << " tmp:" << tmp << endl;
}

int userDatabase::dataFile_add(const string &value)
{
...
void userDatabase::dataFile_delete(const int dataAddress)
{
	int next;
	dataFile.seekg(0);
	dataFile.read(reinterpret_cast<char *>(&next), sizeof(int));
...
void userDatabase::dataFile_replace(const int dataAddress, string &value)
{
	dataFile.seekp(dataAddress);
	dataFile.write(value.c_str(), value.size() + 1);
}

int userDatabase::indexFile_find(int key, int *indexAddress, int *pos, int *size, int *dataAddress)
{
	int current, next = 0;
...
void userDatabase::indexFile_add(const int key, const int dataAddress, const int indexAddress, int pos, int size)
{
...
void userDatabase::indexFile_addAndOverflow(const int key, const int dataAddress, const int indexAddress, const int pos, const int size)
{
...
bool userDatabase::indexFile_delete(const int indexAddress, const int pos, int size)
{
...
void userDatabase::indexFile_borrowLeft(const int indexAddress, int size, const int left, int leftSize, int parent, int parentPosition)
{
...
void userDatabase::indexFile_borrowRight(const int indexAddress, int size, const int right, int rightSize, int parent, int parentPosition)
{
...
void userDatabase::indexFile_mergeLeft(const int indexAddress, const int size, const int left, int leftSize)
{
...
void userDatabase::indexFile_mergeRight(const int indexAddress, int size, const int right, const int rightSize)
{
...
void userDatabase::indexFile_deleteAndUnderflow(const int indexAddress, const int pos, int size)
{
...
bool userDatabase::file_update(const int key, string &value)
{
...
bool userDatabase::select(const int key, string & value)
{
	logFile << endl << "SELECT key:" << key << endl;
	if (ucache.select(key, value))
		return true;
	int indexAddress, pos, size, dataAddress;
	int result = indexFile_find(key, &indexAddress, &pos, &size, &dataAddress);
	...
	int oldKey;
	string oldValue;
	result = ucache.insert(key, value, &oldKey, &oldValue);
	if (result == 2)
		file_update(oldKey, oldValue);
	return true;
}

bool userDatabase::insert(const int key, const string &value)
{
	logFile << endl << "INSERT key:" << key << " value:" << value << endl;
	string fvalue;
	if (ucache.select(key, fvalue))
		return false;
	int indexAddress, pos, size, dataAddress;
	int result = indexFile_find(key, &indexAddress, &pos, &size, &dataAddress);
...
bool userDatabase::remove(const int key)
{
...
bool userDatabase::update(const int key, string &value)
{
	logFile << endl << "UPDATE key:" << key << " value:" << value << endl;
	int result;
	result = ucache.update(key, value);
	if (!result)
	{
