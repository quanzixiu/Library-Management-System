...
{
	scale = 5;
	indexFile.open("userstatesindex.txt", ios::in | ios::out | ios::binary);
	dataFile.open("userstatesdata.txt", ios::in | ios::out | ios::binary);
	logFile.open("userstateslog.txt", ios::trunc);
	if (!indexFile)
	{
		indexFile.close();
		dataFile.close();
		ofstream index("userstatesindex.txt", ios::binary);
		ofstream data("userstatesdata.txt", ios::binary);
		int firstEmpty = 8;
...
void userstatesDatabase::dataFile_find(const int dataAddress, string &value)
{
	char tmp[120];
	dataFile.clear();
...
int userstatesDatabase::dataFile_add(const string &value)
{
...
void userstatesDatabase::dataFile_delete(const int dataAddress)
{
	int next;
	dataFile.seekg(0);
	dataFile.read(reinterpret_cast<char *>(&next), sizeof(int));
...
void userstatesDatabase::dataFile_replace(const int dataAddress, string &value)
{
	dataFile.seekp(dataAddress);
	dataFile.write(value.c_str(), value.size() + 1);
}

int userstatesDatabase::indexFile_find(int key, int *indexAddress, int *pos, int *size, int *dataAddress)
{
	int current, next = 0;
...
void userstatesDatabase::indexFile_add(const int key, const int dataAddress, const int indexAddress, int pos, int size)
{
...
void userstatesDatabase::indexFile_addAndOverflow(const int key, const int dataAddress, const int indexAddress, const int pos, const int size)
{
...
bool userstatesDatabase::indexFile_delete(const int indexAddress, const int pos, int size)
{
...
void userstatesDatabase::indexFile_borrowLeft(const int indexAddress, int size, const int left, int leftSize, int parent, int parentPosition)
{
...
void userstatesDatabase::indexFile_borrowRight(const int indexAddress, int size, const int right, int rightSize, int parent, int parentPosition)
{
...
void userstatesDatabase::indexFile_mergeLeft(const int indexAddress, const int size, const int left, int leftSize)
{
...
void userstatesDatabase::indexFile_mergeRight(const int indexAddress, int size, const int right, const int rightSize)
{
...
void userstatesDatabase::indexFile_deleteAndUnderflow(const int indexAddress, const int pos, int size)
{
...
bool userstatesDatabase::file_update(const int key, string &value)
{
...
bool userstatesDatabase::select(const int key, string & value)
{
	logFile << endl << "SELECT key:" << key << endl;
	if (uscache.select(key, value))
		return true;
	int indexAddress, pos, size, dataAddress;
	int result = indexFile_find(key, &indexAddress, &pos, &size, &dataAddress);
	...
	int oldKey;
	string oldValue;
	result = uscache.insert(key, value, &oldKey, &oldValue);
	if (result == 2)
		file_update(oldKey, oldValue);
	return true;
}

bool userstatesDatabase::insert(const int key, const string &value)
{
	logFile << endl << "INSERT key:" << key << " value:" << value << endl;
	string fvalue;
	if (uscache.select(key, fvalue))
		return false;
	int indexAddress, pos, size, dataAddress;
	int result = indexFile_find(key, &indexAddress, &pos, &size, &dataAddress);
...
bool userstatesDatabase::remove(const int key)
{
...
bool userstatesDatabase::update(const int key, string &value)
{
	logFile << endl << "UPDATE key:" << key << " value:" << value << endl;
	int result;
	result = uscache.update(key, value);
	if (!result)
	{
